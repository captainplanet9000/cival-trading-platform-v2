-- =====================================================
-- Supabase Migration: Wallet-Farm-Goal Integration (COMPATIBLE)
-- Version: 002-compatible
-- Description: Extends existing database with farm/goal management compatible with existing wallets
-- Compatible with existing: wallets, wallet_transactions, agent_* tables
-- =====================================================
...
-- Add farm and goal integration to existing trading_agents table
DO $$
BEGIN
    -- Add columns to existing trading_agents table if they don't exist
...
-- Add farm integration to existing agent_configs table if it exists
DO $$
BEGIN
...
-- Indexes for new agent table columns
CREATE INDEX IF NOT EXISTS idx_trading_agents_farm_id ON trading_agents(farm_id) WHERE farm_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_agent_configs_farm_id ON agent_configs(farm_id) WHERE farm_id IS NOT NULL;
...
DO $$
BEGIN
    -- Farm to wallet foreign key (if wallets table exists)
    ...
    -- Trading agents to farms foreign key
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'trading_agents') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
    ...
    -- Agent configs to farms foreign key
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'agent_configs') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
...
FROM pg_tables 
WHERE tablename IN ('farms', 'goals', 'master_wallet_configs', 'fund_allocations', 'agent_farm_assignments', 'farm_goal_assignments')
ORDER BY tablename;

-- Show existing wallets (to confirm integration)
...
COMMENT ON TABLE farms IS 'Farm management system for organizing trading agents into strategy-based groups - integrated with existing wallet system';
COMMENT ON TABLE goals IS 'Goal tracking system for autonomous trading objectives';
COMMENT ON TABLE master_wallet_configs IS 'Master wallet configurations extending existing wallets table';
COMMENT ON TABLE fund_allocations IS 'Fund allocation tracking between existing wallets and entities';
