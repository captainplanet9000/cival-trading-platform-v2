-- =====================================================
-- Supabase Migration: Wallet-Farm-Goal Integration (FIXED)
-- Version: 001-fixed
...
-- Fund allocations table (no foreign key initially)
...
-- Wallet transactions table (no foreign key initially)
...
-- =====================================================
-- AGENT TABLE UPDATES (Check if table exists)
-- =====================================================

-- Check if agent_configs table exists and add wallet integration columns
DO $$
BEGIN
    -- Only proceed if agent_configs table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'agent_configs') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agent_configs' AND column_name='farm_id') THEN
            ALTER TABLE agent_configs ADD COLUMN farm_id UUID;
        END IF;
        ...
        -- Add foreign key constraint for agent_configs -> farms
        IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                      WHERE constraint_name = 'fk_agent_configs_farm_id' 
                      AND table_name = 'agent_configs') THEN
            ALTER TABLE agent_configs 
                ADD CONSTRAINT fk_agent_configs_farm_id 
...
DO $$
BEGIN
    -- Fund allocations foreign key
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                  WHERE constraint_name = 'fk_fund_allocations_wallet_id' 
    ...
    -- Wallet transactions foreign key
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                  WHERE constraint_name = 'fk_wallet_transactions_wallet_id' 
...
DROP POLICY IF EXISTS "Authenticated users can view master_wallets" ON master_wallets;
DROP POLICY IF EXISTS "Authenticated users can insert master_wallets" ON master_wallets;
DROP POLICY IF EXISTS "Authenticated users can update master_wallets" ON master_wallets;
DROP POLICY IF EXISTS "Authenticated users can delete master_wallets" ON master_wallets;

DROP POLICY IF EXISTS "Authenticated users can view fund_allocations" ON fund_allocations;
DROP POLICY IF EXISTS "Authenticated users can insert fund_allocations" ON fund_allocations;
DROP POLICY IF EXISTS "Authenticated users can update fund_allocations" ON fund_allocations;
DROP POLICY IF EXISTS "Authenticated users can delete fund_allocations" ON fund_allocations;

DROP POLICY IF EXISTS "Authenticated users can view wallet_transactions" ON wallet_transactions;
DROP POLICY IF EXISTS "Authenticated users can insert wallet_transactions" ON wallet_transactions;
DROP POLICY IF EXISTS "Authenticated users can update wallet_transactions" ON wallet_transactions;

DROP POLICY IF EXISTS "Authenticated users can view agent_farm_assignments" ON agent_farm_assignments;
DROP POLICY IF EXISTS "Authenticated users can insert agent_farm_assignments" ON agent_farm_assignments;
DROP POLICY IF EXISTS "Authenticated users can update agent_farm_assignments" ON agent_farm_assignments;
DROP POLICY IF EXISTS "Authenticated users can delete agent_farm_assignments" ON agent_farm_assignments;

DROP POLICY IF EXISTS "Authenticated users can view farm_goal_assignments" ON farm_goal_assignments;
DROP POLICY IF EXISTS "Authenticated users can insert farm_goal_assignments" ON farm_goal_assignments;
DROP POLICY IF EXISTS "Authenticated users can update farm_goal_assignments" ON farm_goal_assignments;
DROP POLICY IF EXISTS "Authenticated users can delete farm_goal_assignments" ON farm_goal_assignments;
...
FROM pg_tables 
WHERE tablename IN ('farms', 'goals', 'master_wallets', 'fund_allocations', 'wallet_transactions', 'agent_farm_assignments', 'farm_goal_assignments')
ORDER BY tablename;
